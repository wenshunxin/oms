...
| stripe | 确认为斑马纹 table | boolean | — | false |
| border | 确认带有纵向边框 | boolean | — | false |
| fit | 列的宽度确认自撑开 | boolean | — | true |
| show-header | 确认显示表头 | boolean | - | true |
| highlight-current-row | 确认要高亮当前行 | boolean | — | false |
...
| fixed | 列确认固定在左侧或者右侧，true 表示固定在左侧 | string, boolean | true, left, right | — |
| render-header | 列标题 Label 区域渲染使用的 Function | Function(h, { column, $index }) | — | — |
| sortable | 对应列确认可以排序，如果设置为 'custom'，则代表用户希望远程排序，需要监听 Table 的 sort-change 事件 | boolean, string | true, false, 'custom' | false |
| sort-method | 对数据进行排序的时候使用的方法，仅当 sortable 设置为 true 的时候有效 | Function(a, b) | — | — |
| resizable | 对应列确认可以通过拖动改变宽度（需要在 el-table 上设置 border 属性为真） | boolean | — | true |
...
| selectable | 仅对 type=selection 的列有效，类型为 Function，Function 的返回值用来决定这一行的 CheckBox 确认可以勾选 | Function(row, index) | — | — |
...
| filter-multiple | 数据过滤的选项确认多选 | Boolean | — | true |
